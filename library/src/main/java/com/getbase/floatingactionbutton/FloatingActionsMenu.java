...
public class FloatingActionsMenu extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        ...
        //同FloatingActionButton里面的size
        mAddButtonSize = attr.getInt(R.styleable.FloatingActionsMenu_fab_addButtonSize, FloatingActionButton.SIZE_NORMAL);
        //同FloatingActionButton里面的StrokeVisible
        mAddButtonStrokeVisible = attr.getBoolean(R.styleable.FloatingActionsMenu_fab_addButtonStrokeVisible, true);
        //按钮列展开方向，up 添加按钮在下按钮列向上弹出 ，right 添加按钮在左按钮列向右弹出
        mExpandDirection = attr.getInt(R.styleable.FloatingActionsMenu_fab_expandDirection, EXPAND_UP);
        //标签文字 的 TextAppearance  如果为0则不会显示label文字
        mLabelsStyle = attr.getResourceId(R.styleable.FloatingActionsMenu_fab_labelStyle, 0);
        //标签文字 的 位置  left or right
        mLabelsPosition = attr.getInt(R.styleable.FloatingActionsMenu_fab_labelsPosition, LABELS_ON_LEFT_SIDE);
        attr.recycle();

        if (mLabelsStyle != 0 && expandsHorizontally()) {
            //当按钮向左或向右弹出时 不支持 设置 style  横向按钮列不会显示label文字
        ...
        switch (mExpandDirection) {
            ...
            case EXPAND_DOWN:
                ...
                for (int i = mButtonsCount - 1; i >= 0; i--) {
                    ...
                    //mExpanded = false平移到添加按钮的位置
                    child.setTranslationY(mExpanded ? expandedTranslation : collapsedTranslation);
                    //mExpanded = false透明度设置为0
                    child.setAlpha(mExpanded ? 1f : 0f);

                    //设置属性动画 目标对象为child 按钮，并加入到动画集合中
                    ...
                    if (label != null) {
                        ...
                        //用于处理label的触摸事件,将child按钮的触摸区域扩展出去到label的范围
